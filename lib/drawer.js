...
Drawer.prototype.eachPoint = function (callback) {
  var x, y;
  for (x = 0; x < this.width; x++) {
    for (y = 0; y < this.height; y++) {
      this.point(x, y, callback.call(this, x, y))
    }
  }
  return this;
}
...
Drawer.prototype.draw = function (callback) {
  callback.call(this)
}
...
Drawer.prototype.scale = function (k, trimW, trimH) {
  ...
  drawer.eachPoint(function (x, y) {
    var pX = Math.floor(x / k)
      , pY = Math.floor(y / k)
    return self.getPointColor(pX, pY)
  })
  return drawer
}
